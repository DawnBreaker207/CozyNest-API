...
export const checkStockBeforePayment: RequestHandler = async (
  req,
  res,
  next,
) => {
  ...
  try {
    const stockCheckResults = await Promise.all(
      cartItems.map(
        async (item: { productId: any; skuId: string; quantity: number }) => {
    ...
    logger.log(
      'error',
      `Catch error in checkStockBeforePayment: ${error.message}`,
      {
        stack: error.stack,
