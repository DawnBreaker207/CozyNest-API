...
const getAllUser: RequestHandler = async (req, res, next) => {
    try {
        ...
        const users = await findAllUser(optionSearch, options);
...
const updateUser: RequestHandler = async (req, res, next) => {
    try {
        const {id} = req.params;
        // Check user exist
        const user = await findExistUser(id);
        if (user.email) {
            // Check email exist
            const emailExist = await findEmailExist(req.body.email);
            if (emailExist._id.toString() !== user._id.toString()) {
        ...
        const newUser = await updateUserInfo(id, req.body);
        res.status(StatusCodes.OK).json({
...
const forgotPass: RequestHandler = async (req, res, next) => {
    try {
        ...
        const findUser = await findEmailExist(email);
        if (!findUser) {
            return res.status(StatusCodes.BAD_REQUEST).json({
                message: messagesError.BAD_REQUEST,
            });
        ...
        const user = await updateUserPassword(email, hashPass);
        if (!user) {
...
const changePassword: RequestHandler = async (req, res, next) => {
    try {
        ...
        const user = updateUserPassword(email, hashPass);
        res.status(StatusCodes.OK).json({
