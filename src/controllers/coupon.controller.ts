...
import {couponCreate, couponGetAll, couponGetOne, couponUpdate} from "@/services/coupon.service";
...
const updateCoupon: RequestHandler = async (req, res, next) => {
    try {
        const {id} = req.params;
        const coupon = await couponGetOne(id);
        coupon.set({...req.body, updateBy: req.user._id});
        await coupon.save();
        await couponUpdate(id, req.body, req.user._id);
        res.status(StatusCodes.OK).json({``
