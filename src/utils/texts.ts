...
const sendOrder = (
  ...
  <div>
      ...
      <div style="border:1px solid #ccc;border-radius:10px; padding:10px 20px;width: max-content">
          ...
          <h4>Tổng: ${
            amountReduced != null
              ? (amountReduced + totalPayment).toLocaleString('vi-VN') + ' VNĐ'
              : formattedTotalPaymentValue // Sử dụng giá trị đã định dạng
          }</h4> 
          <h3 style="color: red;font-weight:bold;margin-top:20px">Tổng tiền thanh toán: ${
            formattedTotalPayment || formattedTotalPaymentValue
          }</h3>
          <p>Thanh toán: ${
            data.pay === false
